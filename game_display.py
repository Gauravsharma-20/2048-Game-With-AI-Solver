#for GUI
from tkinter import Frame, Label, CENTER
#file Imports
import game_ai
import game_functions

###Defining Values---
EDGE_LENGTH = 400
CELL_COUNT = 4
CELL_PAD = 4.2

#KEYS TO USE
UP_KEY = "'w'"
DOWN_KEY = "'s'"
LEFT_KEY = "'a'"
RIGHT_KEY= "'d'"
AI_KEY = "'h'"
AI_PLAY_KEY = "'p'"

#format of Label/text
LABEL_FONT = ("Times", 30, "bold")
#Grid line/padding Color
GAME_COLOR = "#c68c53"# "lemon chiffon"
#Empty cell color
EMPTY_COLOR = "bisque2"
#Color of Tile
TILE_COLORS = {2: "light salmon",4: "orange",8: "tomato",16: "red",32: "yellow2",64: "dark slate gray",
               128: "medium orchid",256: "lawn green",512: "spring green",1024: "thistle1",2048: "gold",
               4096: "lavender",8192: "cyan", }
#Color of Number on tile
LABEL_COLORS = {2: "#011c08", 4: "#011c08", 8: "#011c08", 16: "#011c08",32: "#011c08", 64: "#f2f2f0",
                128: "#011c08",256: "#011c08", 512: "#011c08", 1024: "#011c08",2048: "#f2f2f0",
                4096: "#f2f2f0", 8192: "#011c08", }
###Defining Values---

#Class for GUI
class Display(Frame):
    #initializing the Structure
    def __init__(self):
        #Basic Frame for the GUI
        Frame.__init__(self)
        self.grid()
        self.master.title('2048-Game')

        #associating keys with Functions
        self.master.bind("<Key>", self.key_press)
        self.commands = {UP_KEY: game_functions.move_up, 
                         DOWN_KEY: game_functions.move_down,
                         LEFT_KEY: game_functions.move_left, 
                         RIGHT_KEY: game_functions.move_right,
                         AI_KEY: game_ai.ai_move,
                         }
        #grid intialized
        self.grid_cells = []
        self.build_grid()
        #intializing board
        self.init_matrix()
        self.draw_grid_cells()
        #runs it continuously
        self.mainloop()

    def build_grid(self):
        #make the background and fit it into the grid
        background = Frame(self, bg=GAME_COLOR, width=EDGE_LENGTH, height=EDGE_LENGTH)
        background.grid()

        for row in range(CELL_COUNT):
            grid_row = []
            for col in range(CELL_COUNT):
                cell = Frame(background, bg=EMPTY_COLOR,
                             width=EDGE_LENGTH / CELL_COUNT,
                             height=EDGE_LENGTH / CELL_COUNT)

                cell.grid(row=row, column=col, padx=CELL_PAD,
                          pady=CELL_PAD)
                t = Label(master=cell, text="",
                          bg=EMPTY_COLOR,
                          justify=CENTER, font=LABEL_FONT, width=5, height=2)
                t.grid()
                grid_row.append(t)

            self.grid_cells.append(grid_row)

    #Make the Board Intialize
    def init_matrix(self):
        #empty the board and insert 2 random tiles
        self.matrix = game_functions.initialize_game()

    def draw_grid_cells(self):
        for row in range(CELL_COUNT):
            for col in range(CELL_COUNT):
                tile_value = self.matrix[row][col]
                if not tile_value:
                    self.grid_cells[row][col].configure(
                        text="", bg=EMPTY_COLOR)
                else:
                    self.grid_cells[row][col].configure(text=str(
                        tile_value), bg=TILE_COLORS[tile_value],
                        fg=LABEL_COLORS[tile_value])
        self.update_idletasks()
    
    #Game Controls with Key
    def key_press(self, event):
        valid_game = True
        key = repr(event.char)
        #START AI
        ai_flag = False
        if key == AI_PLAY_KEY:
            move_count = 0
            ai_flag = ~ai_flag
            try:
                while valid_game:
                    self.matrix, valid_game = game_ai.ai_move(
                    self.matrix, 30, 20)  # 40 30
                    if valid_game:
                        self.matrix = game_functions.add_new_tile(self.matrix)
                        self.draw_grid_cells()
                    move_count += 1
            except KeyboardInterrupt:
                pass

        #HINT KEY
        if key == AI_KEY:
            self.matrix, move_made = game_ai.ai_move(self.matrix, 20, 30)
            if move_made:
                self.matrix = game_functions.add_new_tile(self.matrix)
                self.draw_grid_cells()
                move_made = False
        #UP LEFT DOWN RIGHT
        elif key in self.commands:
            self.matrix, move_made, _ = self.commands[repr(event.char)](self.matrix)
            if move_made:
                self.matrix = game_functions.add_new_tile(self.matrix)
                self.draw_grid_cells()
                move_made = False

#START GAME
gamegrid = Display()
